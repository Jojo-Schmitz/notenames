...
It too adds the note names of the top notes in voice 1 of the current score as staff text (above the staff), but works an all staves rather just the top one and uses note names according to the locale MuseScore is configured for rather than just the English note names C, D, E, F, G, A, B.
